...
int main()
{
    char * fifo_w = "/tmp/myfifo";
    int fifo_w_fd = mkfifo(fifo_w, 0666);
    if(fifo_w_fd==-1){
        printf("open fifo_w failed");
        return 0;
    }
    char buf[8] = {'h','e','l','l','o','\0'};
    ssize_t bytes_written = write(fifo_w_fd, buf, sizeof(buf));


    char* shm_name = "feeding";
    int size = 4096;
